const SYSTEM_PROMPT = `All your responses should avoid markdown format and be in plain text only.`;

const ERROR_MESSAGES = {
  GEMINI_UNAVAILABLE: "Gemini is not available.",
...
chrome.commands.onCommand.addListener((command) => {
  ...
  if (command === 'toggle-voice-control') {
    chrome.runtime.sendMessage({ type: 'toggleVoiceControl' });
  }
  if (command === 'toggle-voice-input') {
    chrome.runtime.sendMessage({ type: 'toggleVoiceInput' });
  }
  if (command === 'toggle-repeat') {
    chrome.runtime.sendMessage({ type: 'toggleRepeat' });
  }
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  const handlers = {
    ...
    'startScrollingScreenshot': async () => {
      ...
      return true; // Indicating async response handling
    }
  };
...
// Functions for handling messages and interacting with Gemini
